...
export function DashboardSkeleton() {
  return (
    <div className="px-4 py-6">
      <div className="h-7 skeleton-shimmer rounded w-40 mb-6" />
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="glass rounded-xl p-4 text-center">
            <div className="h-8 skeleton-shimmer rounded w-16 mx-auto" />
            <div className="h-3 skeleton-shimmer rounded w-12 mx-auto mt-2" />
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      <div className="bg-gradient-main px-4 py-6 text-white rounded-b-2xl">
        ...
        <div className="h-4 bg-white/10 rounded w-32 mt-2" />
      </div>
